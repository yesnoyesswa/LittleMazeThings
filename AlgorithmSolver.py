...
def animate_with_trail(maze_np, algo="A*", pause_time=0.01):
    ...
    if algo in ["DFS", "Dijkstra"]:
        ...
        while curr_pos != goal_pos and step < 2000:
            ...
            if not plan or knowledge[plan[0]] == -1: plan = a_star_plan(curr_pos, goal_pos, knowledge)
            if not plan: break
            
            ax1.clear(); ax2.clear()
            ax1.imshow(maze_np, cmap='terrain'); ax1.plot(trail_x, trail_y, 'r-', alpha=0.3)
            ax1.plot(curr_pos[1], curr_pos[0], 'ro', markersize=8)
            ax2.imshow(knowledge, cmap='terrain', vmin=-5, vmax=2)
            ax2.plot(trail_x, trail_y, 'y-', alpha=0.4)
            if plan: py, px = zip(*plan); ax2.plot(px, py, 'c--')
    ...
    if curr_pos == goal_pos:
        ...
        ax2.imshow(display_brain, cmap='terrain', vmin=-5, vmax=2)
...
if __name__ == "__main__":
    ...
    # "DFS", "Dijkstra" hoặc "A*"
    animate_with_trail(maze, algo="A*", pause_time=0.001)